...
import { tierLabels, tierPrices } from "opendiff-components/constants";
import { formatCents, formatDate } from "opendiff-components/utils";
import {
...
export function TeamSubscriptionCard({
  ...
  const pricePerSeat = tierPrices[subscription?.tier ?? "CODE_REVIEW"] ?? 19;
        ...
        {subscription?.status === "ACTIVE" && (
          <>
            ...
            <div className="flex gap-2 mt-6">
              {subscription.cancelAtPeriodEnd ? (
                <Button onClick={onReactivateSubscription}>Reactivate</Button>
              ) : (
                <>
                  <Button
