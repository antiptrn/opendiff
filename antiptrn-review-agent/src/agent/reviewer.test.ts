...
describe("CodeReviewAgent", () => {
  ...
  describe("respondToComment", () => {
    it("should respond to conversation", async () => {
      const conversation = [
        { user: "developer", body: "Why did you flag this?" },
      ];

      mockQueryResult = "This was flagged because of potential security issues.";

      const response = await agent.respondToComment(
        conversation,
        "/tmp/test-repo",
        { filename: "test.ts", diff: "+new code" }
      );
    ...
    it("should handle agent failure in conversation", async () => {
      ...
      await expect(
        agent.respondToComment(conversation, "/tmp/test-repo")
