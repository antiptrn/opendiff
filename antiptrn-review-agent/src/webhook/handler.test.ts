...
class TestableWebhookHandler extends WebhookHandler {
  public testIsCodeFile(filename: string): boolean {
    return (this as any).isCodeFile(filename);
  }

  public testShouldSkipFile(filename: string): boolean {
    return (this as any).shouldSkipFile(filename);
  }
}
...
describe('WebhookHandler file filtering', () => {
  ...
  beforeEach(() => {
    ...
    mockAgent = {};
    mockFormatter = {};
    handler = new TestableWebhookHandler(mockGitHubClient, mockAgent, mockFormatter);
  });
