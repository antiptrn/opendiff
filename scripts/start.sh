...
if [ "$MODE" = "prod" ]; then
  ...
  nohup bun run --cwd "$ROOT/apps/bff" dev > "$LOGS/server.log" 2>&1 &
  nohup bun run --cwd "$ROOT/apps/site" dev > "$LOGS/site.log" 2>&1 &
  nohup bun run --cwd "$ROOT/apps/app" dev > "$LOGS/app.log" 2>&1 &
  nohup bun run --cwd "$ROOT/apps/review-agent" dev > "$LOGS/agent.log" 2>&1 &
fi
...
echo "  site    → http://localhost:5173  (pid: $(lsof -t -i:5173 2>/dev/null || echo 'starting...'))"
