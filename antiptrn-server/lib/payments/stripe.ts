...
const PRICE_ID_TO_TIER: Record<string, "CODE_REVIEW" | "TRIAGE" | "BYOK"> = {
  [process.env.STRIPE_CODE_REVIEW_MONTHLY_PRICE_ID ?? ""]: "CODE_REVIEW",
  [process.env.STRIPE_CODE_REVIEW_YEARLY_PRICE_ID ?? ""]: "CODE_REVIEW",
  [process.env.STRIPE_TRIAGE_MONTHLY_PRICE_ID ?? ""]: "TRIAGE",
  [process.env.STRIPE_TRIAGE_YEARLY_PRICE_ID ?? ""]: "TRIAGE",
  [process.env.STRIPE_BYOK_MONTHLY_PRICE_ID ?? ""]: "BYOK",
  [process.env.STRIPE_BYOK_YEARLY_PRICE_ID ?? ""]: "BYOK",
};

function getPriceIdForTier(tier: "BYOK" | "CODE_REVIEW" | "TRIAGE", billing: "monthly" | "yearly"): string {
...
export const stripeProvider: PaymentProviderInterface = {
  ...
  async previewSubscriptionChange(params: PreviewSubscriptionChangeParams): Promise<SubscriptionChangePreview> {
    const subscription = await getStripe().subscriptions.retrieve(params.subscriptionId);
    ...
    for (const line of upcomingInvoice.lines.data) {
      const isProrationFlag = line.proration === true;
      const description = line.description?.toLowerCase() ?? "";
      // Detect proration line items by description patterns
      const isProrationByDescription = description.includes("unused time") || 
