...
const FROM_EMAIL = process.env.FROM_EMAIL || "antiptrn <noreply@antiptrn.com>";
...
export async function sendInviteEmail({
  ...
  try {
    const { error } = await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: `You've been invited to join ${orgName} on antiptrn`,
      html: `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0;">
    <h1 style="color: white; margin: 0; font-size: 24px;">antiptrn</h1>
  </div>

  <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
    <h2 style="margin-top: 0;">You're invited!</h2>

    <p><strong>${inviterName}</strong> has invited you to join <strong>${orgName}</strong> on antiptrn as a <strong>${role.toLowerCase()}</strong>.</p>

    <p>antiptrn provides AI-powered code reviews for your pull requests, helping you ship better code faster.</p>
    ...
    <p style="color: #6b7280; font-size: 12px;">Or copy this link: <a href="${inviteUrl}" style="color: #667eea;">${inviteUrl}</a></p>
      ...
      `.trim(),
      text: `
${inviterName} has invited you to join ${orgName} on antiptrn as a ${role.toLowerCase()}.

antiptrn provides AI-powered code reviews for your pull requests, helping you ship better code faster.
