...
export class ReviewFormatter {
  ...
  private formatMergeSafety(
    ...
    const issueSummary = this.formatIssueCountSummary(counts);
    if (issueSummary === "no open issues") {
      return "Merge safety was not generated by the reviewer for this run.\n\n";
    }

    return `Merge safety was not generated by the reviewer for this run. Review the ${issueSummary} listed below before merging.\n\n`;
  }

  private normalizeGeneratedMergeSafety(mergeSafety?: string): string | null {
    const normalized = mergeSafety?.trim();
    if (!normalized) {
      return null;
    }
