...
import {
  ...
  type TimeInterval,
  useReviewsOverTime,
} from "../hooks/use-reviews-over-time";
...
/** Area chart comparing current and previous period review metrics with configurable time intervals. */
    ...
    <Card className="pb-2">
      <CardHeader className="flex flex-row items-start justify-between -mb-1">
        <Select value={timeInterval} onValueChange={(v) => setTimeInterval(v as TimeInterval)}>
          <SelectTrigger
            variant="ghost"
            className="-ml-5 -mt-3 !bg-transparent !ring-0 hover:text-muted-foreground"
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
