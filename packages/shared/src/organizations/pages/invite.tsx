...
export default function InvitePage() {
  ...
  // Auto-switch to the invited account if user has multiple accounts
  useEffect(() => {
    if (!invite?.invitedEmail || !accounts || accounts.length <= 1) return;

    const matchingAccount = accounts.find(
      (account) => account.email?.toLowerCase() === invite.invitedEmail?.toLowerCase()
    );

    if (matchingAccount && user?.email?.toLowerCase() !== invite.invitedEmail?.toLowerCase()) {
      // Switch to the account that was invited
      switchAccount(matchingAccount.visitorId || matchingAccount.id);
    }
  }, [invite?.invitedEmail, accounts, user?.email, switchAccount]);
