...
async function handleCheckoutCompleted(checkout: NormalizedCheckout) {
  ...
  if (metadata?.type === "org_subscription" && metadata.orgId && subscriptionId) {
    ...
    // If org doesn't exist (may have been deleted), skip
    if (!org) {
      console.log(
        `[${getPaymentProviderName()}] Checkout completed but org ${metadata.orgId} not found, skipping (likely deleted)`
      );
      return;
    }
    ...
    if (org.subscriptionId === subscriptionId && org.subscriptionStatus === "ACTIVE") {
      console.log(
        `[${getPaymentProviderName()}] Checkout completed for org ${metadata.orgId} - subscription already configured, skipping`
      );
      return;
    }
    ...
    let seatCount = Number.parseInt(metadata.seatCount ?? String(quantity), 10) || 1;
    ...
    console.log(
      `[${getPaymentProviderName()}] Checkout completed for org ${metadata.orgId}: ${tier}, ${seatCount} seats`
    );
    return;
  }
  ...
  if (customerEmail && subscriptionId) {
    ...
    if (user) {
      ...
      console.log(
        `[${getPaymentProviderName()}] Activated legacy subscription for user ${user.login}: ${tier}`
      );
    }
  }
}
...
async function handleSubscriptionUpdated(subscription: NormalizedSubscription) {
  ...
  if (metadata?.type === "org_subscription" && metadata.orgId) {
    // Check if the organization still exists (may have been deleted)
    ...
    if (!existingOrg) {
      console.log(
        `[${getPaymentProviderName()}] Organization ${metadata.orgId} not found, skipping subscription update (likely deleted)`
      );
      return;
    }
    ...
    if (isNewSubscription && mappedStatus === "ACTIVE" && quantity > 0) {
      ...
      if (membersToAssign.length > 0) {
        ...
        console.log(
          `[${getPaymentProviderName()}] Auto-assigned ${membersToAssign.length} seat(s) for new subscription`
        );
      }
    }

    console.log(
      `[${getPaymentProviderName()}] Updated org subscription for ${metadata.orgId}: ${tier}, ${quantity} seats, status: ${mappedStatus}`
    );
    return;
  }
  ...
  if (org) {
    await prisma.organization.update({
      ...
      data: {
        subscriptionTier:
          mappedStatus === "ACTIVE" ? (tier as SubscriptionTier) : org.subscriptionTier,
        subscriptionStatus: mappedStatus,
        productId: productId,
        seatCount: quantity,
        cancelAtPeriodEnd,
    ...
    console.log(
      `[${getPaymentProviderName()}] Updated org subscription by ID lookup: ${tier}, ${quantity} seats`
    );
    return;
  }
  ...
  if (user) {
    await prisma.user.update({
      ...
      data: {
        subscriptionTier:
          mappedStatus === "ACTIVE" ? (tier as SubscriptionTier) : user.subscriptionTier,
        subscriptionStatus: mappedStatus,
        subscriptionId: subscriptionId,
        productId: productId,
        cancelAtPeriodEnd,
    ...
    console.log(
      `[${getPaymentProviderName()}] Updated legacy user subscription: ${tier}, status: ${mappedStatus}`
    );
  }
}
...
async function handleSubscriptionCanceled(subscription: NormalizedSubscription) {
  ...
  if (org) {
    ...
    console.log(
      `[${getPaymentProviderName()}] Org subscription scheduled for cancellation: ${org.slug}`
    );
    return;
  }
  ...
  if (user) {
    ...
    console.log(
      `[${getPaymentProviderName()}] Legacy subscription scheduled for cancellation: ${user.login}`
    );
  }
}
...
async function handleSubscriptionRevoked(subscription: NormalizedSubscription) {
  ...
  if (org) {
    ...
    console.log(
      `[${getPaymentProviderName()}] Org subscription revoked, all seats unassigned: ${org.slug}`
    );
    return;
  }
