...
/** Custom tooltip for Recharts that displays a formatted label and color-coded data entries. */
      ...
      <div className="mt-1 space-y-1.5">
        {payload.map((entry) => (
          <div key={entry.dataKey} className="flex items-center gap-1.5 text-sm">
            ...
            <span className="ml-auto text-xs pl-6 text-foreground tabular-nums">{entry.value}</span>
          </div>
        ))}
      </div>
    </div>
